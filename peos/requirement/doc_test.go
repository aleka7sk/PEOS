...
// peos/relation.
//
// peos/relation is permitted -- and required -- because PEOS-005 §17.4 binds
// every Requirement relationship to PEOS-002's Artifact Relation contract, so
// each of this package's six relationship wrappers composes relation.Relation
// rather than duplicating its fields.
//
// peos/lifecycle is excluded because PEOS-005 §26 states Requirement lifecycle
// is "governed exclusively by PEOS-003" and this package models none of it;
// peos/validation and peos/quality because PEOS-005 §30.1 keeps Requirements
// independent of validation and quality outcomes; peos/decision because a
// Waiver names its governance basis as core.AuthorityRef and a
// core.DecisionOutcomeRef, never a decision.Outcome value.
func TestProductionImportBoundary(t *testing.T) {
	allowed := map[string]bool{peosModulePrefix + "core": true, peosModulePrefix + "relation": true}
...
func TestTestImportBoundary(t *testing.T) {
	allowed := map[string]bool{peosModulePrefix + "core": true, peosModulePrefix + "relation": true}
	// A package's own external test files (package requirement_test) legitimately
	...
	allowed[peosModulePrefix+"requirement"] = true
...
// TestValidationDoesNotImportRequirement locks the converse direction and is
// the mirror of peos/validation's own TestRequirementDoesNotImportValidation.
...
// Requirement; neither package may name the other's types.
func TestValidationDoesNotImportRequirement(t *testing.T) {
	assertDoesNotImport(t, filepath.Join("..", "validation"), "peos/validation",
		peosModulePrefix+"requirement",
		"PEOS-006 reaches Requirements through core.EngineeringSubjectRef and core.CriterionRef, never through PEOS-005 types")
}
