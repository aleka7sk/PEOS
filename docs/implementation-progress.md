...
| PEOS-005 — Requirement Model | `[~]` | `peos/requirement` (`Requirement`, `Revision`, `Content`, `Subject`, `Statement` — Packet C; `Derivation` — Packet G.1, **corrected by G.1.1.I**; `Refinement`, `Decomposition` — Packet G.2; `RequirementParticipant`, `Dependency`, `Conflict` — Packet G.3.I; `GovernanceAction`, `LifecycleConsequence`, `Supersession` — Packet G.4.I) + `peos/relation` (dependency, introduced by G.1). **Authoritative architecture plan: Packet G.B** | Packet C (§6–§16); Packet G/G.A/G.B (architecture); Packet G.1 (§17, §17.1–§17.4, §18, §18.1–§18.2 — foundation + Derivation); Packet G.2 (§19, §19.1, §20, §20.1–§20.2 — Refinement + Decomposition); **Packet G.1.1/G.1.1.I (Derivation §18 identity-distinctness audit and correction, complete and accepted)**; **Packet G.3/G.3.I (§21, §21.1, §22, §22.1 — Dependency + Conflict, complete and accepted)**; **Packet G.4/G.4.I (§23, §23.1, §23.2 — Requirement Supersession, complete and accepted)** | **G.5 — Waiver.** No Allocation implementation packet (Allocation is Product-owned) | Packet G.A: **BLUEPRINT APPROVED WITH REQUIRED REVISIONS** (2 MAJOR, 7 MINOR, 2 NOTE, all incorporated by G.B). Packet G.B: **PACKET G BLUEPRINT REVISED AND ACCEPTED.** Packets G.1 and G.2: implemented per their Blueprints. **Packet G.1.1: DERIVATION ARTIFACT ID DISTINCTNESS REQUIRED** (normative audit; genuine conformance gap found in G.1). **Packet G.1.1.I: correction applied and verified**, no architectural deviation, all quality gates pass. **Packet G.3: Blueprint READY FOR IMPLEMENTATION** (participant-level union architecture, Dependency/Conflict asymmetry, all 16 required sections resolved). **Packet G.3.I: implemented exactly per Blueprint**, no architectural deviation, all quality gates pass. **Packet G.4: Blueprint READY FOR IMPLEMENTATION** (revision-level participants, identity-preservation-not-distinctness verdict, §26.5 lifecycle/relationship separation, all 20 required sections resolved). **Packet G.4.I: implemented exactly per Blueprint**, no architectural deviation, all quality gates pass | `go test ./peos/requirement` passes, coverage 95.2% (verified 2026-07-26; up from 95.1%, new files at 91–100% per-function). **Minimum conformance (§34.1): now 10 of 10** — §34.1's tenth bullet ("preserve explicitly represented Requirement relationships and their engineering meaning") is satisfied now that all six Requirement Relation Types (§17.4: Derivation, Refinement, Decomposition, Dependency, Conflict, Supersession) are implemented. Waiver (§27, Packet G.5) is not one of §34.1's ten bullets and does not reopen this count. **Requirement-identity distinctness is required by Derivation and Decomposition only; Refinement, Dependency, Conflict, and Supersession all permit two Revisions of one Requirement identity to participate together**, each for its own stated reason (see `doc.go`'s five-way comparison). No §18 identity question remains open, and Supersession's identity question (the packet's central risk, per the Blueprint) is resolved and recorded |
...
| G.5 | `[ ]` | Waiver (§27, §27.1–§27.2, §36.11) — no-identity value record, independent of G.1–G.4 | Not started | — | — | G.4.I (satisfied — `GovernanceAction` is explicitly reusable, unmodified, for §27's identically-worded authorization clause) |
...
> **Packet G.5 — Waiver Blueprint** (PEOS-005 §27, §27.1–§27.2, §36.11)
>
> Design Waiver, the last outstanding PEOS-005 concept scheduled in the Packet G roadmap.
> Per the Packet G.B binding architecture (still authoritative, not reopened by G.1–G.4):
> Waiver has **no identity**, is **not** an Artifact Relation (it does not compose
> `peos/relation.Relation`, and must not reuse `relationship.go`'s helpers or the nested
> relation JSON envelope), and is a plain value record owned by `peos/requirement`. §27's
> authorization clause ("A waiver SHALL be authorized by an established Decision Outcome or
> by another governance mechanism explicitly permitted by an applicable PEOS Product
> contract") is worded identically to §23's, so this packet is expected to reuse
> `GovernanceAction` (`governance.go`, introduced by G.4.I) directly and unmodified — do not
> define a second, Waiver-specific governance-action type. Confirm from §27.1/§27.2 exactly
> what Waiver's own required fields are (authority, scope, and whatever else those clauses
> mandate) before finalizing the type layout.

- **Recommended model:** Claude Opus
- **Recommended mode:** Plan (blueprint phase); exit Plan Mode only once the Waiver shape is approved
- **Expected result:** A Packet G.5 Blueprint at the same level of detail as Packets G.1's, G.2's, G.3's, and G.4's, ready for direct implementation with no further architectural decisions. Once G.5/G.5.I complete, every PEOS-005 concept scheduled in the Packet G roadmap will be implemented (Allocation remains `PROD`, requiring no SDK packet).

**Historical / superseded next actions — do not act on these:** every next action recorded
in this section before Packet G.4.I (the original "Packet G — conformance-matrix audit
phase," "Packet G.1 — … Blueprint," "Packet G.2 — … Blueprint," the G.1.1/G.1.1.I
audit-and-correction gate, "Packet G.3 — Dependency + Conflict Blueprint," and "Packet G.4 —
Requirement Supersession Blueprint") is now complete and superseded. **Packet G.5, above, is
the sole current next action in this document, and it is unblocked.**
